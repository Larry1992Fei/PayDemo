  ...
  return (
    <div className="flex h-full flex-col bg-slate-50">
      <div className="border-b border-slate-100 bg-white px-5 py-4">
        <h3 className="text-[15px] font-black text-slate-900">后续商户扣款</h3>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
        ...
        <div className="grid grid-cols-2 gap-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <Info label="扣款金额" value={`${currency} ${amount}`} />
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-2 break-all rounded-xl border border-slate-100 bg-slate-50 p-3 font-mono text-[11px] text-slate-700">
            {mandateTokenId || '请先完成首次绑定并通过 orderQuery 获取 paymentTokenID'}
          </div>
        </div>
        ...
        {lastApiResponse && (
          <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
            ...
            <div>
              <div className="text-xs font-bold uppercase tracking-wide text-emerald-800">绑定结果已完成</div>
